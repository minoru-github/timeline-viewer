...
- 依存作成: 依存元モジュールを Shift キーを押しながらクリックしてから、依存先モジュールを Shift 押しつつクリックします（2 回目のクリックで A → B が作成されます）。
...
- モジュール編集: タイムライン上のモジュールを Ctrl+クリックすると編集モーダルが開きます。変更後に `保存` を押すと反映されます。
